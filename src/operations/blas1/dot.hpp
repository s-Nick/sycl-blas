...
Dot<lhs_t, rhs_t>::Dot(lhs_t &_l, rhs_t &_r, rhs_t& _rhs2)
    : lhs_(_l), rhs_(_r), rhs2_(_rhs2){};

template <typename lhs_t, typename rhs_t>
PORTBLAS_INLINE typename Dot<lhs_t, rhs_t>::index_t
Dot<lhs_t, rhs_t>::get_size() const {
  return rhs_.get_size();
}

template <typename lhs_t, typename rhs_t>
PORTBLAS_INLINE bool Dot<lhs_t, rhs_t>::valid_thread(
    cl::sycl::nd_item<1> ndItem) const {
  return true;
}

template <typename lhs_t, typename rhs_t>
PORTBLAS_INLINE typename Dot<lhs_t, rhs_t>::value_t
Dot<lhs_t, rhs_t>::eval(cl::sycl::nd_item<1> ndItem) {
  ...
  for (int id = lid; id < size;
    ...
    in_val += rhs_.eval(lid)*rhs2_.eval(lid);
  }
  ...
  return {};  // Dot<lhs_t, rhs_t>::eval(ndItem.get_global_id(0));
}
template <typename lhs_t, typename rhs_t>
template <typename sharedT>
PORTBLAS_INLINE typename Dot<lhs_t, rhs_t>::value_t
Dot<lhs_t, rhs_t>::eval(sharedT scratch, cl::sycl::nd_item<1> ndItem) {
  auto atomic_res = sycl::atomic_ref<value_t, sycl::memory_order::relaxed,
                                     sycl::memory_scope::device,
                                     sycl::access::address_space::global_space>(
      lhs_.get_data()[0]);
...
template <typename lhs_t, typename rhs_t>
PORTBLAS_INLINE void Dot<lhs_t, rhs_t>::bind(cl::sycl::handler &h) {
  lhs_.bind(h);
  rhs_.bind(h);
  rhs2_.bind(h);
}
...
template <typename lhs_t, typename rhs_t>
PORTBLAS_INLINE void Dot<lhs_t, rhs_t>::adjust_access_displacement() {
  lhs_.adjust_access_displacement();
  rhs_.adjust_access_displacement();
  rhs2_.adjust_access_displacement();
}
}  // namespace blas
