...
void run_test(const combination_t<scalar_t> combi) {
  ...
  for (int i = 0; i < batch_size; ++i) {
    reference_blas::omatcopy(trans, m, n, alpha, A_ref.data() + (i*stride_a), ld_in,
        B_ref.data() + (i*stride_b), ld_out);
  }

  auto matrix_in = blas::make_sycl_iterator_buffer<scalar_t>(A, size*batch_size);
  auto matrix_out = blas::make_sycl_iterator_buffer<scalar_t>(B, size*batch_size);

  blas::extension::_omatcopy_batch(sb_handle, trans, m, n, alpha, matrix_in, ld_in, stride_a,
                             matrix_out, ld_out, stride_b, batch_size);

  auto event = blas::helper::copy_to_host<scalar_t>(sb_handle.get_queue(),
                                                    matrix_out, B.data(), size*batch_size);
...
const auto combi = ::testing::Combine(
    ::testing::Values<char>('n'), ::testing::Values<int64_t>(64, 128, 256),
